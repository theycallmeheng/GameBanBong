...
/**
 * BAN BONG ULTRA (1 file)
 * - UI xịn: parallax starfield, vignette, aura, nổ hạt, glow
 * - Máy bay có vệt khói + lửa phản lực
 * - Trạng thái: START / PLAYING / PAUSED / GAMEOVER
 * - Điều khiển: ← → , SPACE bắn, P tạm dừng, ENTER bắt đầu/chơi lại
 * - Tương thích: NetBeans 8 + JDK7 (Windows XP)
 */
public class GameBanBong extends JPanel implements ActionListener, KeyListener {
    ...
    // ===== Data classes
    static class Bullet {
        int x,y,w,h,speed; Bullet(int x,int y,int w,int h,int s){this.x=x;this.y=y;this.w=w;this.h=h;this.speed=s;}
    }
    static class Ball {
        int x,y,r,vx,vy; Color color;
        Ball(int x,int y,int r,int vx,int vy, Color c){this.x=x;this.y=y;this.r=r;this.vx=vx;this.vy=vy;this.color=c;}
    }
    static class Particle {
        float x,y,vx,vy,life=1f; Color color;
        Particle(float x,float y,float vx,float vy, Color c, float life){this.x=x;this.y=y;this.vx=vx;this.vy=vy;this.color=c;this.life=life;}
    }
    static class Trail {
        float x,y,vy,life=1f,size=10f;
        Trail(float x,float y,float dx, float vy,float life){this.x=x+dx;this.y=y;this.vy=vy;this.life=life;}
    }
    static class Star {
        int x,y; float speed; Star(int x,int y,float s){this.x=x;this.y=y;this.speed=s;}
    }
